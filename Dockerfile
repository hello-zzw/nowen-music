...
ENV MYSQL_PASSWORD Zzw123456
ENV DATABASE_NAME no_wen
...
# 将构建产物jar包拷贝到运行时目录中
COPY --from=build /app/target/now-music-0.0.1.jar .

# 暴露端口
EXPOSE 80

# 执行启动命令
CMD ["java", "-jar", "/app/now-music-0.0.1.jar", "--spring.profiles.active=test"]